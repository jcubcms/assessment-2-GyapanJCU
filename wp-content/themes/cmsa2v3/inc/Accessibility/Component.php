...
class Component implements Component_Interface {
	...
	public function action_enqueue_navigation_script() {
		...
		wp_enqueue_script(
			...
			[],
			cmsa2v3()->get_asset_version( get_theme_file_path( '/assets/js/navigation.min.js' ) ),
			false
		);
		wp_script_add_data( 'cmsa2v3-navigation', 'async', true );
		wp_script_add_data( 'cmsa2v3-navigation', 'precache', true );
		wp_localize_script(
			'cmsa2v3-navigation',
			'cmsa2v3ScreenReaderText',
			[
				'expand'   => __( 'Expand child menu', 'cmsa2v3' ),
				'collapse' => __( 'Collapse child menu', 'cmsa2v3' ),
			]
		);
	}

...
use function WP_Rig\WP_Rig\cmsa2;
...
class Component implements Component_Interface, Templating_Component_Interface {
	...
	/**
	 * Gets template tags to expose as methods on the Template_Tags class instance, accessible through `cmsa2()`.
	 *
	...
	public function action_enqueue_styles() {
		...
		if ( ! empty( $google_fonts_url ) ) {
			wp_enqueue_style( 'cmsa2-fonts', $google_fonts_url, [], null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		}
		...
		foreach ( $css_files as $handle => $data ) {
			...
			$version = cmsa2()->get_asset_version( $css_dir . $data['file'] );
	...
	public function filter_resource_hints( array $urls, string $relation_type ) : array {
		if ( 'preconnect' === $relation_type && wp_style_is( 'cmsa2-fonts', 'queue' ) ) {
			$urls[] = [
		...
		$handles   = array_filter(
			...
			function( $handle ) use ( $css_files ) {
				...
				if ( ! $is_valid ) {
					...
					_doing_it_wrong( __CLASS__ . '::print_styles()', esc_html( sprintf( __( 'Invalid theme stylesheet handle: %s', 'cmsa2' ), $handle ) ), 'cms a2 2.0.0' );
				}
				return $is_valid;
			}
		);
	 ...
	 * {@see 'cmsa2_preloading_styles_enabled'} filter can be used to tweak the return value.
	 *
	...
	protected function preloading_styles_enabled() {
		$preloading_styles_enabled = ! cmsa2()->is_amp();
		 ...
		 */
		return apply_filters( 'cmsa2_preloading_styles_enabled', $preloading_styles_enabled );
	}
	...
	protected function get_css_files() : array {
		...
		$css_files = [
			...
			'cmsa2-sidebar'    => [
				...
				'preload_callback' => function() {
					return cmsa2()->is_primary_sidebar_active();
				},
			],
			'cmsa2-widgets'    => [
		 ...
		 */
		$css_files = apply_filters( 'cmsa2_css_files', $css_files );
	...
	protected function get_google_fonts() : array {
		 ...
		 */
		$this->google_fonts = (array) apply_filters( 'cmsa2_google_fonts', $google_fonts );

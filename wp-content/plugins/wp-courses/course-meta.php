...
function wpc_add_course_lessons_meta_box() {
	...
	foreach ( $screens as $screen ) {
		add_meta_box(
			'wpc_course_lessons',
			__( 'Manage Course Lessons', 'wp-courses' ),
...
function wpc_course_lessons_meta_box_callback( $post ) {
	...
	echo $wpc_admin->get_lesson_list($post->ID);
}
...
function wpc_course_meta_box_callback( $post ) {
	...
	<label for="wpc_new_field"><?php _e( 'Video Embed Code (iframe)', 'wp-courses' ); ?></label>
	<textarea style="width:100%;" id="wpc_video_id" name="wpc_video_id"><?php echo $value; ?></textarea>
	<br><label><?php echo __('Course Teacher (Hold Ctrl for Multiple)', 'wp-courses'); ?>:</label><br>
	<?php
	global $post;
	$post_old = $post;
	...
	echo '<select id="teacher-select" name="teacher-selection[]" multiple>';
	...
	echo '</select>';
	$admin_url = admin_url();
	echo '<br><a href="' . $admin_url . 'post-new.php?post_type=teacher" class="page-title-action add-new button" style="margin-top: 8px">' . __('Add New', 'wp-courses') . '</a>';
	...
	?>
	<br><label><?php echo __('Course Category', 'wp-courses'); ?>:</label>
	<div class="tagsdiv" id="<?php echo $tax_name; ?>">
	    ...
	    <?php echo ' <a href="' . $admin_url . 'edit-tags.php?taxonomy=course-category&post_type=course" class="page-title-action add-new button">' . __('Add New', 'wp-courses') . '</a>'; ?>
	    </div>
	</div>
	<?php 
    ...
    $taxonomy = get_taxonomy($tax_name);
	?>
	<label><?php echo __('Course Difficulty', 'wp-courses'); ?>:</label>
	<div class="tagsdiv" id="<?php echo $tax_name; ?>">
	    <div class="jaxtag">
	    <?php 
	    // Use nonce for verification
	    ...
	    wp_dropdown_categories('taxonomy=course-difficulty&hide_empty=0&orderby=name&name=course-difficulty&show_option_none=Select Difficulty&selected='. $id); ?>
	   	<?php echo ' <a href="' . $admin_url . 'edit-tags.php?taxonomy=course-difficulty&post_type=course" class="page-title-action add-new button">' . __('Add New', 'wp-courses') . '</a>'; ?>
	    </div>
	</div>
<?php }
/**
...
function wpc_course_save_meta_box_data( $post_id ) {
	...
	if(isset($_POST['teacher-selection'])){
		$teacher = $_POST['teacher-selection'];
